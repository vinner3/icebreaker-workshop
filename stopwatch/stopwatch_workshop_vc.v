...
module top (
	...
	// Lap registers
	reg [7:0] lap_value = 0;     // holds lap snapshot
	reg [4:0] lap_timeout = 0;   // counts down laps (20 = 2 seconds)
	...
	always @(posedge CLK) begin
		...
		// Reset or increment timer
		if (!BTN_N) begin
			display_value <= 8'd0;          // synchronous reset when BTN_N is pressed (active-low)
			running <= 0;
			lap_timeout <= 0;
		...
		// Stop/Start Buttons
		if (BTN3) begin
			running <= 1;          // synchronous reset when BTN_N is pressed (active-low)
		end 
		...
		// Decrement lap_timeout every clkdiv_pulse (if > 0)
		if (clkdiv_pulse && lap_timeout > 0) begin
			lap_timeout <= lap_timeout - 1;
		end

		// Lap button BTN2: snapshot and start lap_timeout
		if (BTN2) begin
			lap_value <= display_value;
			lap_timeout <= 5'd20;   // show lap for 20 cycles (~2 seconds)
		end
	...
	seven_seg_ctrl seven_segment_ctrl (
		...
		.din(lap_timeout > 0 ? lap_value : display_value),
		.dout(seven_segment)
	);
